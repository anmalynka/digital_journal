...
const Block: React.FC<BlockProps> = ({ block, dragHandleProps }) => {
  ...
  const renderIcon = () => {
    if (isLocked) return <Lock className="w-4 h-4 text-journal-muted" />;
    
    switch (block.type) {
      case 'heading1': return <Heading1 className="w-4 h-4 text-journal-text" />;
      case 'heading2': return <Heading2 className="w-4 h-4 text-journal-muted" />;
      case 'checklist': return (
        <div 
          onClick={toggleStatus}
          className={`w-5 h-5 rounded-md border-2 transition-all flex items-center justify-center cursor-pointer
            ${block.status === 'done' ? 'bg-journal-accent border-journal-accent' : 'border-journal-muted hover:border-journal-text bg-white'}
          `}
        >
          {block.status === 'done' && <Check className="w-3 h-3 text-white stroke-[4px]" />}
        </div>
      );
      case 'callout': return <AlertCircle className="w-5 h-5 text-journal-yellow" />;
      case 'image': return <ImageIcon className="w-5 h-5 text-journal-muted" />;
      case 'file': return <FileIcon className="w-5 h-5 text-journal-muted" />;
      case 'canvas': return <PenTool className="w-5 h-5 text-journal-purple" />;
      case 'timer': return <Clock className="w-5 h-5 text-journal-error" />;
      default:
        if (block.type === 'event') return <Circle className="w-3 h-3 text-journal-text fill-current" />;
        if (block.type === 'note') return <Minus className="w-3 h-3 text-journal-muted" />;
        switch (block.status) {
          case 'done': return <Check className="w-3 h-3 text-journal-accent stroke-[3px]" />;
          case 'migrated': return <ArrowRight className="w-3 h-3 text-journal-yellow stroke-[3px]" />;
          case 'scheduled': return <ArrowLeft className="w-3 h-3 text-journal-muted stroke-[3px]" />;
          case 'cancelled': return <X className="w-3 h-3 text-journal-error/50 stroke-[3px]" />;
          default: return <div className="w-1.5 h-1.5 bg-journal-text rounded-full" />;
        }
    }
  };

  const renderContent = (text: string) => {
    if (isLocked) return <span className="italic text-journal-muted">This memory is currently sealed...</span>;
    
    return text.split(/(\[\[.*?\]\]|#\w+)/g).map((part, i) => {
      if (part.startsWith('[[') && part.endsWith(']]')) {
        const target = part.slice(2, -2);
        return (
          <button key={i} onClick={(e) => { e.stopPropagation(); navigatetoLink(target); }} className="text-journal-accent font-black hover:underline">{target}</button>
        );
      }
      if (part.startsWith('#')) return <span key={i} className="text-journal-muted font-bold">{part}</span>;
      return part;
    });
  };
  ...
  return (
    <div 
      className={`group relative flex items-start gap-3 py-2 px-4 transition-all rounded-2xl
        ${block.status === 'done' ? 'bg-journal-accent/5 opacity-80' : 'hover:bg-black/5'}
        ${block.type.startsWith('heading') ? 'mt-6 mb-2' : ''}
        ${block.type === 'callout' ? 'bg-journal-yellow/10 border border-journal-yellow/20 p-6 my-4' : ''}
        ${block.type === 'timer' ? 'my-8' : ''}
      `}
      style={{ marginLeft: `${block.indent * 32}px` }}
    >
      <div {...dragHandleProps} className="mt-1.5 p-1 text-journal-muted opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing hover:text-journal-text outline-none" role="button" tabIndex={0}>
      ...
      <div className="flex-1 relative min-h-[1.5rem]">
        ...
        {block.type === 'file' && block.fileData && (
          <div className="flex items-center gap-3 p-4 bg-journal-card border border-black/5 rounded-2xl shadow-sm mb-4">
            <FileIcon className="w-8 h-8 text-journal-muted" />
            <div className="flex flex-col">
              <span className="text-sm font-bold text-journal-text">{block.fileName}</span>
              <span className="text-[10px] text-journal-muted uppercase font-black">{(block.fileData.size / 1024).toFixed(1)} KB</span>
            </div>
          </div>
        )}
        ...
        {(!isFocused && !['image', 'file', 'canvas', 'timer'].includes(block.type)) && (
          <div className={`py-1 px-1 leading-relaxed whitespace-pre-wrap break-words
            ...
            ${block.status === 'done' ? 'text-journal-accent line-through opacity-60' : 'text-journal-text'}
          `} onClick={() => setIsFocused(true)}>
            {renderContent(content) || <span className="text-journal-muted/40 italic">...</span>}
          </div>
        )}
        ...
        <AnimatePresence>
          {showSlashMenu && (
            <motion.div 
              ...
              exit={{ opacity: 0 }}
              className="absolute left-0 top-full z-[100] mt-2 w-64 bg-journal-card rounded-4xl shadow-2xl border border-black/5 p-2"
            >
              <div className="text-[10px] font-black text-journal-muted uppercase tracking-widest p-3 border-b border-black/5 mb-1">Basic Blocks</div>
              <div className="max-h-64 overflow-auto custom-scrollbar">
                {blockTypes.map((bt) => (
                  <button
                    key={bt.type}
                    onClick={() => changeType(bt.type)}
                    className="w-full flex items-center gap-3 p-3 hover:bg-black/5 rounded-3xl transition-all text-left group"
                  >
                    <div className="w-8 h-8 rounded-2xl bg-black/5 flex items-center justify-center group-hover:bg-white transition-colors">
                      <bt.icon className="w-4 h-4 text-journal-text" />
                    </div>
                    <span className="text-sm font-bold text-journal-text">{bt.label}</span>
                  </button>
                ))}
              </div>
            </motion.div>
      ...
      <button onClick={() => deleteBlock(block.id)} className="mt-1.5 p-1 text-journal-muted opacity-0 group-hover:opacity-100 hover:text-journal-error transition-opacity">
